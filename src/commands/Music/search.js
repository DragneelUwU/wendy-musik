...
module.exports = {
   ...
   execute: async (message, args, client) => { 
    ...
    const but4 = new MessageButton().setCustomId("s_four").setLabel("4").setStyle("PRIMARY");
    const but5 = new MessageButton().setCustomId("s_five").setLabel("5").setStyle("PRIMARY");
             ...
             const results = s.tracks.slice(0, 5).map(x => `☯ ${index++} | [${x.title}](${x.uri}) \`${convertTime(x.duration)}\``)
                    ...
                    .setAuthor({
                       name:`Here are your search results for ${query}`,
                        ...
                        //.setTitle("ere re your results for")
                        .setColor(client.embedColor)
