...
module.exports = {
    ...
    execute: async (message, args, client, prefix) => {
        ...
        const embed = new MessageEmbed()
            ...
            .setTimestamp()
            .setDescription(`Choose what filter from the below buttons`)
            .setFooter({
                text:`Please wait for the filter to apply`,
        ...
        const row2 = new MessageActionRow().addComponents( but7, but8, but9);

        const m = await message.reply({ embeds: [embed], components: [row, row2] });

        const embed1 = new MessageEmbed().setColor(client.embedColor).setTimestamp().setFooter({text:``});
        ...
        collector.on("end", async () => {
            if (!m) return;
            await m.edit({ embeds: [embed1.setDescription(`Collecter has timed out, use  \`${prefix}filters\` again`)] });
        });
